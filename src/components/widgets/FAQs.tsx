      ...
      <div class="px-4 py-16 mx-auto max-w-6xl lg:py-20">
        ...
        <div class="max-w-screen-xl sm:mx-auto">
          <div class="grid grid-cols-1 gap-x-8 gap-y-8 lg:gap-x-16 md:grid-cols-2">
            {items.map((subitems, itemIdx) => (
              <div class="space-y-8" key={itemIdx}>
                {subitems.map(({ question, answer }, subItemIdx) => (
                  <div  key={subItemIdx}>
                    <h3 class="mb-4 text-xl font-bold">
                      <IconArrowDownRight class="w-7 h-7 text-secondary-500 inline-block" />
                      {question}
                    </h3>
                    {answer.split("\n\n").map((paragraph, answerKey) => (
                      <p class="text-gray-700 dark:text-gray-400 mb-2" key={answerKey}>
                        {paragraph}
                      </p>
                    ))}
                  </div>
